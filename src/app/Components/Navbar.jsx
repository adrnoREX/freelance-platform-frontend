...
function Navbar() {
  ...
  return (
    <>
      <div className="sm:w-full sticky top-0 shadow z-[999] bg-white text-black transition-all duration-500 px-20 py-4  flex items-center justify-between">
        <div>
                  ...
                  {dropdownOpen && (
                    <div className="absolute right-0 mt-2 w-48 bg-white border border-gray-200 rounded-xl shadow-md z-[999] text-gray-700 py-3 px-4 flex flex-col gap-3 text-sm">
                      ...
                      {currentUser.role === "freelancer" && (
                        <>
                          <Link href="/freelancer/add-gig" className="cursor-pointer">
                            Add New Gig
                          </Link>
                        </>
                      )}
      ...
      <div
        className={`sm:hidden fixed top-0 left-0 h-full w-3/4 bg-white shadow-lg z-50 transform transition-transform duration-300 ease-in-out ${
                ...
                <div className="flex flex-col gap-4">
                  <div className="flex items-center gap-3">
                    <img
                      src={
                        currentUser.img
                          ? `http://localhost:8800/uploads/${encodeURIComponent(currentUser.img)}`
                          : "/img/noavatar.jpg"
                      }
                      alt="avatar"
                  ...
                  {currentUser.role === "freelancer" && <Link href="/freelancer/add-gig">Add New Gig</Link>}
                  {pathname !== "/orders" && (
                    <Link href="/orders" onClick={() => handleContinue()}>
                      Orders
                    </Link>
